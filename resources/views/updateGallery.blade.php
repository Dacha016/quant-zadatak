...
@section("content")
    ...
    @if(($_SESSION["role"]==="moderator" && $result->userId === $_SESSION["id"]) || $_SESSION["role"]==="admin" || ($_SESSION["role"]==="user" && $result->userId === $_SESSION["id"]) )
        ...
        <form action="/update/gallery/{{$result->slug}}" method="post"
            ...
            <div class="mb-3 pt-3 ">
                ...
                <input type="text" class="form- control d-block p-2"
                       style="width:95%;margin:10px auto; border-radius:10px" id="name" name="name"
                       value="{{$result->name}}">
            </div>
            ...
            <div class="m-3">
                @if(($_SESSION["role"]==="admin" && $result->userId !== $_SESSION["id"]))
                    <a class="btn btn-secondary d-inline-block" style="color: white"
                       href="/profile/users/{{$result->userUsername}}?page={{$_GET['page']}}">Cancel</a>
                @endif
                @if(($_SESSION["role"]==="admin" && $result->userId == $_SESSION["id"]))
                    <a class="btn btn-secondary d-inline-block" style="color: white"
                       href="/profile/galleries?page={{$_GET['page']}}">Cancel</a>
                @endif
